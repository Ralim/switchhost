...
func maketestServer(t *testing.T) (*Server, *library.Library, string) {
	...
	server := NewServer(lib, titledb, 8080, 8021)
...
func TestHTTPServerbasics(t *testing.T) {
	...
	if response != `{"files":[],"directories":null,"success":"SwitchRoooooot","titledb":{}}{"files":[{"url":"http://test/vfile/365418291444842496/0/data.bin#UnitTest [05123A0000000000][v0].nsp","size":1,"title":"test"}],"directories":null,"success":"SwitchRoooooot","titledb":{}}` {
		t.Errorf("response doesnt match expected >%s<", response)
	}
